...
class SpecialEventsActivity : BaseNavActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (activeEvent) {
            ...
            is SpecialEventManager.SpecialEvent.Tombolata -> setupTombolataUI(root)
        }
    ...
    private fun setupTombolataUI(root: LinearLayout) {
        val c = this
        val status = TombolataManager.getStatus(c)

        val progressLabel = TextView(c).apply {
            text = "\uD83C\uDF86 ${status.totalDaysClaimed}/5 giorni giocati"
        ...
        val mvcLabel = TextView(c).apply {
            text = "\uD83D\uDCB0 Totale: ${status.totalMvcEarned} MVC guadagnati"
            textSize = 12f; setTextColor(Color.parseColor("#FFD700"))
            gravity = Gravity.CENTER
            setPadding(0, 0, 0, dp(12))
        }
        root.addView(mvcLabel)
        ...
        val progressBarBg2 = View(c).apply {
            layoutParams = FrameLayout.LayoutParams(LP_MW, dp(8))
            background = GradientDrawable().apply {
                cornerRadius = dp(4).toFloat()
                setColor(Color.parseColor("#1A1030"))
        ...
        progressWrapper.addView(progressBarBg2)

        val progressBarFill2 = View(c).apply {
            background = GradientDrawable().apply {
                cornerRadius = dp(4).toFloat()
                setColor(Color.parseColor("#FF6F00"))
            }
        }
        progressWrapper.addView(progressBarFill2)
        root.addView(progressWrapper)

        val progressPct = (status.totalDaysClaimed * 100 / 5).coerceIn(0, 100)
        progressBarFill2.post {
            val parentWidth = (progressBarFill2.parent as FrameLayout).width
            if (parentWidth > 0) {
                progressBarFill2.layoutParams = FrameLayout.LayoutParams(
                    parentWidth * progressPct / 100, dp(8)
                )
            }
        }

        root.addView(UiKit.section(c, "Tombolata di Capodanno"))

        val rewards = TombolataManager.getRewardsCalendar()
        val todayDay = TombolataManager.getTombolataDay()

        val calendarGrid = LinearLayout(c).apply {
            orientation = LinearLayout.VERTICAL
        }
        ...
        for (reward in rewards) {
            ...
            val borderColor = when {
                isClaimed -> Color.parseColor("#00FF88")
                isToday -> Color.parseColor("#FF6F00")
                else -> Color.parseColor("#332244")
            }
            cell.post {
                cell.foreground = GradientDrawable().apply {
                    cornerRadius = dp(10).toFloat()
                    setStroke(dp(if (isToday || isClaimed) 2 else 1), borderColor)
                }
            }
            ...
            if (reward.item != null) {
                textCol.addView(TextView(c).apply {
                    text = "${TombolataManager.itemEmojiForDay(day)} ${reward.item}"
                    textSize = 11f; setTextColor(Color.parseColor("#FFD700"))
                    setPadding(0, dp(2), 0, 0)
                })
            }
            ...
            inner.addView(TextView(c).apply {
                text = "+${reward.mvc}"
                textSize = 12f; setTextColor(Color.parseColor("#00FF88"))
                typeface = Typeface.create("sans-serif-medium", Typeface.BOLD)
            })
            ...
            if (isToday) {
                ...
                cell.isFocusable = true
                cell.setOnClickListener {
                    val intent = android.content.Intent(c, TombolataActivity::class.java)
                    intent.putExtra("day", todayDay)
                    startActivity(intent)
                }
                ...
                cell.isFocusable = true
                cell.setOnClickListener {
                    val msg = "Giorno $day: +${reward.mvc} MVC${if (reward.item != null) "\nItem: ${reward.item}" else ""}"
                    Toast.makeText(c, msg, Toast.LENGTH_SHORT).show()
                }
            }
        ...
        root.addView(TextView(c).apply {
            text = "Tocca un giorno per giocare alla tombola!"
            textSize = 11f; setTextColor(Color.parseColor("#4A3870"))
            gravity = Gravity.CENTER
            setPadding(0, dp(16), 0, dp(8))
        })
