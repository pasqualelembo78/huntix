...
/**
 * EventsActivity — pagina eventi unificata.
 *
 * Unisce Eventi Live (attivi/in arrivo), Eventi Speciali (calendario ElfHunt)
 * ed eventi giornalieri (programma settimanale) in un'unica schermata.
 */
class EventsActivity : BaseNavActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val scroll = ScrollView(c).apply { setBackgroundColor(Color.parseColor("#0D0620")) }
        ...
        root.addView(UiKit.title(c, "Eventi", "\uD83C\uDF89"))

        // ═══ SEZIONE 1: EVENTI LIVE ═══
        root.addView(sectionTv("⚡ Eventi Live", "#FF9800"))

        activeMultiplierLabel = TextView(c).apply {
            textSize = 13f; setTextColor(Color.parseColor("#E0E0FF")); gravity = Gravity.CENTER
            setPadding(0, 0, 0, dp(12))
        }
        root.addView(activeMultiplierLabel)

        eventsContainer = LinearLayout(c).apply { orientation = LinearLayout.VERTICAL }
        root.addView(eventsContainer)

        root.addView(sectionTv("⏳ In Arrivo", "#00B4FF").also { it.setPadding(0, dp(18), 0, dp(8)) })

        upcomingContainer = LinearLayout(c).apply { orientation = LinearLayout.VERTICAL }
        root.addView(upcomingContainer)

        // ═══ SEZIONE 2: EVENTI SPECIALI E GIORNALIERI ═══
        root.addView(sectionTv("🎄 Eventi Speciali e Giornalieri", "#FFD700").also { it.setPadding(0, dp(18), 0, dp(8)) })
        buildSpecialSection(root)
    ...
    private fun buildEventCard(event: LiveEvent, active: Boolean): CardView {
        ...
        val titleCol = LinearLayout(this).apply {
            orientation = LinearLayout.VERTICAL
            layoutParams = LinearLayout.LayoutParams(0, LP_WW, 1f).also { it.marginStart = dp(8) }
        }
        ...
        if (active) {
            val pb = ProgressBar(this, null, android.R.attr.progressBarStyleHorizontal).apply {
                ...
                layoutParams = LinearLayout.LayoutParams(LP_MW, dp(6)).also { it.topMargin = dp(10) }
            }
            col.addView(pb)
        }
    ...
    // ─────────────────────────────────────────────────────────────
    // EVENTI SPECIALI E GIORNALIERI
    // ─────────────────────────────────────────────────────────────

    private fun buildSpecialSection(root: LinearLayout) {
        val c = this
    ...
    private fun setupElfHuntUI(root: LinearLayout) {
        ...
        val progressWrapper = FrameLayout(c).apply {
            layoutParams = LinearLayout.LayoutParams(LP_MW, dp(8)).also { it.bottomMargin = dp(16) }
        }
        ...
        calendarContainer = LinearLayout(c).apply { orientation = LinearLayout.VERTICAL }
    ...
    private fun refreshCalendarIfActive() {
        if (::calendarContainer.isInitialized) refreshCalendar()
    }
    ...
    private fun refreshCalendar() {
        ...
        runOnUiThread {
            ...
            progressBarFill.post {
                ...
                if (parentWidth > 0) {
                    progressBarFill.layoutParams = FrameLayout.LayoutParams(parentWidth * progressPct / 100, dp(8))
                }
            }
            ...
            for (day in 1..25) {
                ...
                val cell = CardView(c).apply {
                    ...
                    setCardBackgroundColor(Color.parseColor(bgColor))
                    layoutParams = LinearLayout.LayoutParams(0, dp(72), 1f).also { it.marginEnd = dp(4) }
                }
    ...
    private fun setupDailyEventUI(root: LinearLayout, event: com.intelligame.huntix.gamification.DailyEvent) {
        ...
        if (isActive && event.activityClass != null) {
            val playBtn = LinearLayout(c).apply {
                ...
                layoutParams = LinearLayout.LayoutParams(LP_WW, LP_WW).apply { topMargin = dp(8) }
                isClickable = true; isFocusable = true
                setOnClickListener { startActivity(Intent(c, event.activityClass)) }
            }
        ...
        for (evt in allEvents) {
            ...
            val eventRow = LinearLayout(c).apply {
                ...
                layoutParams = LinearLayout.LayoutParams(LP_MW, LP_WW).also { it.bottomMargin = dp(4) }
            }
